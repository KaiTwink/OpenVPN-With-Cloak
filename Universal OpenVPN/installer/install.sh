...
# ==================== НОВЫЕ ПЕРЕМЕННЫЕ ДЛЯ XOR И CLOAK ====================
XOR_ENABLED="no"
CLOAK_ENABLED="no"
OPENVPN_INTERNAL_PORT=1194
XOR_PASSWORD=""
OPENVPN_BIN="/usr/sbin/openvpn"
CK_SERVER_BIN="/usr/local/bin/ck-server"
WORKDIR="$(mktemp -d "/tmp/openvpn-install.XXXXXX")"
...
# ==================== ВОПРОСЫ К ПОЛЬЗОВАТЕЛЮ (самое начало) ====================
ask_xor_cloak() {
  read -r -p "Нужен ли XOR Scramble для OpenVPN? (y/N) " reply
  [[ "${reply:-}" =~ ^[Yy]$ ]] && XOR_ENABLED="yes"

  read -r -p "Нужен ли Cloak (ck-server) как дополнительный слой? (y/N) " reply
  [[ "${reply:-}" =~ ^[Yy]$ ]] && CLOAK_ENABLED="yes"
}
...
install_packages() {
  export DEBIAN_FRONTEND=noninteractive
  if [[ "$XOR_ENABLED" == "yes" ]]; then
    apt-get install -y build-essential pkg-config autoconf automake libtool patch \
      zlib1g-dev libssl-dev liblz4-dev liblzo2-dev libpam0g-dev libpkcs11-helper1-dev \
      libsystemd-dev libcap-ng-dev curl unzip jq
  else
    apt-get install -y openvpn easy-rsa
  fi
  apt-get install -y sqlite3 docker.io python3 python3-venv python3-pip \
    curl ca-certificates iptables-persistent netfilter-persistent unzip jq
  systemctl enable --now docker || true
}

# ==================== УСТАНОВКА OPENVPN С XOR ====================
install_openvpn_with_xor() {
  log "Компиляция OpenVPN с XOR Scramble (Tunnelblick-патч)..."
  apt-get install -y easy-rsa build-essential pkg-config autoconf automake libtool patch ca-certificates curl unzip \
    zlib1g-dev libssl-dev liblz4-dev liblzo2-dev libpam0g-dev libpkcs11-helper1-dev libsystemd-dev libcap-ng-dev

  local tb_zip="${WORKDIR}/Tunnelblick-master.zip"
  curl -fsSL -o "$tb_zip" "https://github.com/Tunnelblick/Tunnelblick/archive/refs/heads/master.zip"
  unzip -q "$tb_zip" -d "$WORKDIR"
  local tb_root="${WORKDIR}/Tunnelblick-master"

  local v=2.6.13
  local patch_dir="${tb_root}/third_party/sources/openvpn/openvpn-${v}/patches"
  if [[ ! -d "$patch_dir" ]]; then
    v=2.6.11
    patch_dir="${tb_root}/third_party/sources/openvpn/openvpn-${v}/patches"
  fi

  local tarball="${WORKDIR}/openvpn-${v}.tar.gz"
  curl -fsSL -o "$tarball" "https://swupdate.openvpn.org/community/releases/openvpn-${v}.tar.gz"
  tar -xzf "$tarball" -C "$WORKDIR"

  pushd "${WORKDIR}/openvpn-${v}" >/dev/null
  cp "${patch_dir}"/*.diff .
  for p in 02-tunnelblick-openvpn_xorpatch-*.diff; do
    patch -p1 < "$p" || die "Не удалось применить патч $p"
  done
...
# ==================== УСТАНОВКА CLOAK ====================
install_cloak() {
  log "Установка Cloak (ck-server) на порт 443..."
  ...
  local api_json tag asset_name asset_url
  api_json="$(curl -fsSL https://api.github.com/repos/cbeuw/Cloak/releases/latest)"
  tag="$(jq -r '.tag_name' <<<"$api_json")"
  asset_name="ck-server-linux-${arch}-${tag}"
  asset_url="$(jq -r --arg n "$asset_name" '.assets[] | select(.name==$n) | .browser_download_url' <<<"$api_json")"

  curl -fsSL -o "${WORKDIR}/ck-server" "$asset_url"
  install -m 0755 "${WORKDIR}/ck-server" /usr/local/bin/ck-server
  CK_SERVER_BIN="/usr/local/bin/ck-server"

  local key_out="$("$CK_SERVER_BIN" -key 2>&1)"
  CLOAK_PRIVATE_KEY="$(printf '%s' "$key_out" | awk '/Private/ {print $NF; exit}')"
  local uid_out="$("$CK_SERVER_BIN" -uid 2>&1)"
  CLOAK_UID="$(printf '%s' "$uid_out" | awk 'NF{print $NF}' | head -n1)"

  mkdir -p /etc/cloak
  cat > /etc/cloak/ckserver.json <<EOF
{
  "ProxyBook": { "openvpn": ["udp", "127.0.0.1:${OPENVPN_INTERNAL_PORT}"] },
  "BindAddr": [":443"],
  "BypassUID": ["${CLOAK_UID}"],
  "RedirAddr": "www.cloudflare.com",
...
[Service]
ExecStart=${CK_SERVER_BIN} -c /etc/cloak/ckserver.json
Restart=always
[Install]
WantedBy=multi-user.target
EOF
  systemctl daemon-reload
  systemctl enable --now ck-server.service
}
...
write_server_config() {
  local bind=""
  if [[ "$CLOAK_ENABLED" == "yes" ]]; then
    bind="local 127.0.0.1"
    OVPN_PORT="$OPENVPN_INTERNAL_PORT"
  fi
...
CONF
  ...
  if [[ "$XOR_ENABLED" == "yes" ]]; then
    XOR_PASSWORD="$(openssl rand -hex 16)"
    echo "scramble obfuscate ${XOR_PASSWORD}" >> "$SERVER_DIR/server.conf"
  fi
}
