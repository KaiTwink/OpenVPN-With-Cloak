...
install_openvpn_with_xor() {
  log "Компиляция OpenVPN с XOR Scramble..."
  ...
  local v=2.6.13
  local patch_files=($(find "$tb_root" -name "*xorpatch*.diff" | sort -u))
  ...
  for p in "${patch_files[@]}"; do
    cp -n "$p" . 2>/dev/null || true
    local patch_file=$(basename "$p")
    patch -p1 < "$patch_file" || die "Не удалось применить патч $patch_file"
  done
  ...
  log "OpenVPN с XOR успешно скомпилирован"
}

install_cloak() {
  log "Установка Cloak (ck-server)..."
...
main() {
  ...
  if [[ "$XOR_ENABLED" == "yes" ]]; then
    ...
    log "Используем обычный OpenVPN из репозитория"
  fi
  collect_inputs
  copy_project
  setup_python
